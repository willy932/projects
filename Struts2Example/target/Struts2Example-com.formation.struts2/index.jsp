...
app.controller("Controller",function($scope,$http){
	...
	$scope.SaveArticle=function(){
		$http({
	       	 method : "GET",
	       	 url : "save",
	       	 params :{'titre':$scope.titre,'description':$scope.description,'image':$scope.image,'contents':$scope.contenue,'status':'false'}
	    }).then(function mySucces(response) {
	        $scope.showMe = response.data.status;
	        $scope.ListerArticle();
	    }, function myError(response) {
...
<table class="table" >
	...
	<tbody>
		...
		<td>{{c.contents}}</td>
		<td><button ng-click="DeleteArticle(c.id)" class="btn btn-primary">Delete</button></td>
